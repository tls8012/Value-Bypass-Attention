...
from configs import WANDB_SQUAD, notify, KINDS, SEEDS, MAX_CONCURRENCY
...
def postprocess_qa_predictions(raw_examples, features, raw_predictions,
                               n_best_size=20, max_answer_length=40):
    ...
    for ex in raw_examples:
        ...
        for fi in feat_indices:
            ...
            start_best = np.argsort(s_log)[-n_best_size:][::-1]
            end_best   = np.argsort(e_log)[-n_best_size:][::-1]
            ...
            for s in start_best:
                for e in end_best:
                    if s <= e and (e - s + 1) <= max_answer_length:
                        ...
                        valid.append({"text": text, "score": score})"""
            # (교체) postprocess_qa_predictions(...) 안, per-feature 루프 부분
        ...
        pred_text = "" if not valid else max(valid, key=lambda x: x["score"])["text"]
        predictions.append({"id": ex_id, "prediction_text": pred_text})
        # 정답은 원본 그대로(여러 개 가능)
...
def do_this(k, SEED):
    ...
    wandb.init(project=WANDB_SQUAD, name=RUN_NAME, group=k)
    trainer.train()
